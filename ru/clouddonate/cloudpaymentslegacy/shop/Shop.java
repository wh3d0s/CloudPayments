    ...
    private void checkPayments() {
        try {
            ...
            if (conn.getResponseCode() != 200) {
                if (Config.Settings.debug) plugin.getLogger().warning("Не удалось получить данные: " + conn.getResponseCode());
                return;
            }
            ...
            plugin.getLogger().severe("[CloudPayments] Ошибка при получении данных о магазине: " + e.getMessage());
        }
    }
        ...
        if (!commands.isEmpty()) {
            plugin.getServer().getScheduler().runTask(plugin, () -> {
                for (String command : commands) {
                    try {
                        if (Config.Settings.debug) plugin.getLogger().info("Выполнено " + command);
                        Bukkit.dispatchCommand(Bukkit.getConsoleSender(), command);
                    } catch (Exception e) {
                        plugin.getLogger().warning("Ошибка при выполнение: " + command);
                        e.printStackTrace();
                    }
                }
            });
    ...
    private void approvePayment(GetResult data) {
        try {
            ...
            if (conn.getResponseCode() == 204) {
                ...
                plugin.getLogger().warning("Ошибка при подтверждение ID " + data.getId() + ". Код: " + conn.getResponseCode());
            }
        } catch (Exception e) {
            plugin.getLogger().warning("Ошибка при подтверждение покупки " + data.getId());
        }
    }
